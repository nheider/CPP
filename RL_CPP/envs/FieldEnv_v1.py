...
class FieldEnv(gym.Env):
    ...
    def random_point_on_polygon_perimeter(self): # To determin the start point and heading of the agent 
        perimeter = 0
        for i in range(len(self.sg_field_poly.coords)):
            x1, y1 = self.sg_field_poly.coords[i]
            x2, y2 = self.sg_field_poly.coords[(i + 1) % len(self.sg_field_poly.coords)]
            perimeter += np.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)

        random_distance = np.random.uniform(0, perimeter)
        cumulative_distance = 0

        for i in range(len(self.sg_field_poly.coords)-1):
            x1, y1 = self.sg_field_poly.coords[i]
            x2, y2 = self.sg_field_poly.coords[(i + 1) % len(self.sg_field_poly.coords)]
    ...
    def steering_to_curve(self, distance, steering_angle):
        ...
        cover_polygon = left_edge + list(reversed(right_edge))
        self.sg_cover_poly = sg.Polygon(cover_polygon)

        if self.sg_path_poly == None: 
            self.sg_path_poly = self.sg_cover_poly
        else: 
    ...
    def calc_intersect_area(self, poly_1, poly_2):
        draw(poly_1)
        draw(poly_2)
        intersect_area = abs(float(boolean_set.intersect(poly_1, poly_2).area()))
        return intersect_area
    
    def check_if_completed(self):
        return (abs(float(self.sg_field_poly.area())) == self.calc_intersect_area(self.sg_field_poly, self.sg_path_poly))
    ...
    def get_lidar_distances(self):
        ...
        for i in range(self.num_lidar_rays):
            ...
            for j in range(len(self.sg_field_poly.coords)):
                p1 = np.array(self.sg_field_poly.coords[j])
                p2 = np.array(self.sg_field_poly.coords[(j + 1) % len(self.sg_field_poly.coords)])
    ...
    def check_if_inside(self):
        inside = False 
        print(self.sg_field_poly.oriented_side(sg.Point2(self.path[-1])))  
        #    inside = True
        return inside
